...
run_smoke_e2e() {
  ...
  local dev_bundle_dir="${root_dir}/smoke-dev"
  ...
  rm -rf "${smoke_dir}"
  mkdir -p "${bundle_dir}" "${dev_bundle_dir}" "${dead_bundle_dir}" "${rootfs}" "${layer}" "${upper}" "${work}"
  ...
  droplet spec \
    ...
    --rootfs "${rootfs}" \
    --cwd "/" \
    --command "/bin/sh -c 'echo smoke-dev'" \
    --ns "mount" --ns "uts" \
    --hostname "smoke-dev" \
    --if_name "" --if_addr "" \
    --image_layer "${layer}" \
