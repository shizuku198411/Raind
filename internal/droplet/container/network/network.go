...
// Controller sets up a veth pair, attaches it to the
...
func (c *Controller) Prepare(containerId string, pid int, annotation spec.AnnotationObject) error {
...
func (c *Controller) createVethPair(containerId string, pid int, networkConfig spec.NetConfigObject, rootless bool) error {
...
func (c *Controller) setupContainerNetns(containerId string, pid int, networkConfig spec.NetConfigObject, rootless bool) error {
	pidStr := fmt.Sprint(pid)
	containerVethName := BuildContainerVethPeerName(containerId)
