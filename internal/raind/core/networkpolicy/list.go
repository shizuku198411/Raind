...
func (s *ServiceList) printList(list []NetworkPolicyInfo) {
	w := tabwriter.NewWriter(os.Stdout, 0, 0, 2, ' ', tabwriter.DiscardEmptyColumns)
	fmt.Fprintln(w, "NETWORKPOLICY ID\tNAME\tNAMESPACE\tINGRESS\tEGRESS\tGENERATED\tCREATED")
	for _, np := range list {
		fmt.Fprintf(w, "%s\t%s\t%s\t%d\t%d\t%d\t%s\n", np.NetworkPolicyId, np.Name, np.Namespace, np.IngressRules, np.EgressRules, np.GeneratedRules, formatTime(np.CreatedAt))
	}
	w.Flush()
}
